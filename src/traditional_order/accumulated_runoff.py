...
for csv_file in csv_files:
    ...
    for main_riv_b in df_b['MAIN_RIV'].unique():
        ...
        grouped = rows_a.groupby('ORD_STRA').agg({'RUNOFF':'sum','CATCH_SKM':'sum'}).reset_index()

        # 遍历分组结果，将每个 ordstra 对应的累加 runoff 添加到结果 DataFrame 中
        for index, row in grouped.iterrows():
            result_df = result_df.append({'MAIN_RIV': main_riv_b, 'ORD_STRA': row['ORD_STRA'], 'Total_RUNOFF': row['RUNOFF'],'Total_CATCH':row['CATCH_SKM']}, ignore_index=True)

# 将结果保存到 CSV 文件
result_df.to_csv('/data_seagate/zhaocs/data/hydroatlas/data_class/accumulated_runoff.csv', index=False)